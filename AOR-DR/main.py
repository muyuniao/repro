...
class SimpleFusion(nn.Module):
    def __init__(self, feature_dim=768, context_dim=1, emb_dim=768, num_classes=5):
        super(SimpleFusion, self).__init__()
        self.position_embedding = nn.Embedding(num_classes - 1, emb_dim)

        #affine fusion
        self.proj_feature = nn.Linear(feature_dim, emb_dim)
        self.proj_context = nn.Embedding(num_classes, emb_dim)
        # cross-attn
    ...
    def __init__(self, images_root, data_file, transforms):
        ...
        if data_file.endswith('.csv'):
            ...
            for idx, row in df.iterrows():
                self.images_file.append(row['image_path'])
...
def prepare_model(chkpt_dir, arch='vit_large_patch16'):
    ...
    print('-------------------------------model-------------------------',msg)
    return model
def prepare_model_vit(arch='vit_base_patch16_224', pretrained=True, pretrained_path=None):
    # 使用 timm 加载预训练的 ViT-B 模型
    if pretrained_path is not None and os.path.exists(pretrained_path):
        print(f"Loading local ViT weights from {pretrained_path}")
        ...
        if pretrained_path.endswith('.safetensors'):
            ...
            print("Local safetensors loaded:", msg)
        else:
            state_dict = torch.load(pretrained_path, map_location='cpu')
            if 'model' in state_dict:
                state_dict = state_dict['model']
            msg = model.load_state_dict(state_dict, strict=False)
            print("Local pth loaded:", msg)
    else:
...
class ViTFeatureExtractor(nn.Module):
    def __init__(self, vit_model,
        ...
        self.SimpleFusion = SimpleFusion(num_classes=num_classes)
...
def main():
    parser = argparse.ArgumentParser(description="AOR-DR Reproduction")
    parser.add_argument('--num_classes', type=int, default=5, help="Number of classes (default: 5)")
    parser.add_argument('--images_root', type=str, default=r'C:\Users\yuqinkai\Downloads\GDRBench_AllowedShare\FundusDG_mini\images\messidor', help="Root path for images")
    parser.add_argument('--data_file', type=str, default=r'C:\Users\yuqinkai\Downloads\GDRBench_AllowedShare\FundusDG_mini\splits\MESSIDOR_train.txt', help="Path to train data split file")
    parser.add_argument('--val_data_file', type=str, default=r'C:\Users\yuqinkai\Downloads\GDRBench_AllowedShare\FundusDG_mini\splits\MESSIDOR_crossval.txt', help="Path to validation data split file")
    parser.add_argument('--vit_pretrained_path', type=str, default=None, help="Path to local ViT pretrained weights")
    parser.add_argument('--batch_size', type=int, default=64, help="Batch size (default: 64)")
    parser.add_argument('--epochs', type=int, default=300, help="Total training epochs (default: 300)")
    parser.add_argument('--lr', type=float, default=1e-4, help="Learning rate (default: 1e-4)")
    parser.add_argument('--dry_run', action='store_true', help="Run only 1 batch to verify pipeline sanity")
    parser.add_argument('--eval_only', action='store_true', help="Only run evaluation and skip training")
    parser.add_argument('--weights_path', type=str, default=None, help="Path to trained model weights for evaluation")
    parser.add_argument('--fold', type=int, default=0, help="Fold index (default: 0)")
    parser.add_argument('--no_notify', action='store_true', help="Disable automatic notification script trigger")
    args = parser.parse_args()

    num_classes = args.num_classes
    steps = num_classes - 1

    vit_base_patch16_224 = prepare_model_vit(pretrained_path=args.vit_pretrained_path)
    feature_extractor = ViTFeatureExtractor(vit_base_patch16_224, num_classes=num_classes)
    ...
    if args.weights_path is not None and os.path.exists(args.weights_path):
        print(f"Loading feature extractor weights from {args.weights_path}")
    ...
    train_dataset = RegressionDataset(images_root=args.images_root,
    ...
    valid_dataset1 = RegressionDataset(images_root=args.images_root,
    ...
    for epoch in range(args.epochs):
        if not args.eval_only:
            ...
            for batch_idx, (image, targets) in enumerate(train_loader):
                ...
                total_loss = 0
                vectors_targets = label_to_vector(targets, max_length=steps)
                ...
                if args.dry_run:
                    print("Dry-run: Completed 1 training batch successfully.")
                    break
            print(f"Epoch {epoch + 1}, Loss: {total_cost / len(train_loader)}")
            log_name = f'vit-b_adience_fold{args.fold}' if num_classes == 8 else f'vit-b_aptos_fold{args.fold}'
        ...
        for image, targets in valid_loader1:
            ...
            with torch.no_grad():
                ...
                for t in range(steps):
                    if t == 0:
                        ...
                        fusion_feature  = feature_extractor.SimpleFusion(features,torch.clamp(torch.round(probas), 0, 1),t)
                        all_probas.append(probas)
                    else:
                        probas,fusion_feature = feature_extractor.sample(fusion_feature,t,temperature=1.0,cfg=1.0)
                        fusion_feature  = feature_extractor.SimpleFusion(fusion_feature,torch.clamp(torch.round(probas), 0, 1),t)
                        all_probas.append(probas)
                all_probas = [torch.round(p) for p in all_probas]
                all_probas_tensor = torch.stack(all_probas).squeeze(-1)  # 变成 [64, steps]
                all_probas_tensor = torch.stack(all_probas).transpose(0, 1)  # 变成 [64, steps]
            ...
            if args.dry_run:
                print("Dry-run: Completed 1 validation batch successfully.")
                break
        accuracy2 = correct / total if total > 0 else 0
        ...
        if args.eval_only:
            ...
            return
        txt_log_name = f'vit-b_adience_eval_fold{args.fold}' if num_classes == 8 else f'vit-b_messidor_fold{args.fold}'
        with open(f'{txt_log_name}.txt', 'a') as f:
            f.write(f'Epoch {epoch}, accuracy: {accuracy2:.4f},f1: {f1:.4f},mae: {mae:.4f},sensitivity: {np.mean(sensityvity):.4f},specificity: {np.mean(specificity):.4f}\n')
        if accuracy2 > best_accuracy2:
            best_accuracy2 = accuracy2
            weight_save_name = f'./vit-b_adience_position_fold{args.fold}.pth' if num_classes == 8 else f'./vit-b_messidor_position_fold{args.fold}.pth'
            torch.save(feature_extractor.state_dict(), weight_save_name)
            print(f"Saved new best model with accuracy: {best_accuracy2:.4f}")
        if args.dry_run:
            print("Dry-run pipeline sanity check passed successfully!")
            return
...
# 在模型初始化后调用
    ...
    print('average sensitivity',np.mean(sensityvity))
    print('average specificity',np.mean(specificity))
    accuracy1 = correct / total if total > 0 else 0
    f1 = f1 / len(valid_loader2) if len(valid_loader2) > 0 else 0
    k = k / len(valid_loader2) if len(valid_loader2) > 0 else 0
    print('-------------------------------accuracy-------------------------')
...
if __name__ == '__main__':
    try:
        ...
        if '--no_notify' not in sys.argv:
            try:
                import os
                import subprocess
                # 兼容地在当前目录或父目录查找并运行 notification.py
                for path in ['../notification.py', './notification.py']:
                    if os.path.exists(path):
                        print(f"训练完成，正在运行通知脚本: {path}")
