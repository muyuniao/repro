...
class DiffLoss(nn.Module):
    ...
    def forward(self, target, z):
        ...
        loss = loss_dict["loss"]
        probas = loss_dict["model_output"]
        # if mask is not None:
        #     loss = (loss * mask).sum() / mask.sum()
        return loss.mean(), probas

    def sample(self, z, temperature=1.0, cfg=1.0, sampler="ddpm", eta=0.0):
        ...
        if not cfg == 1.0:
            ...
            assert torch.isfinite(z).all(), "z contains NaN or Inf values."
            # print('z',z.shape)
